...
def model_factory(C, D, X, N, drop_path, weights,
                  img_size=224, num_classes=1000, distillation=False, pretrained=False, fuse=False,
                  patch_size=16):
...
def levit_picker(arch, dataset, num_classes):
    if arch in specification:
        spec = specification[arch]
    else:
        raise ValueError('Invalid LeVit [{dataset}] model chosen: {arch}')
    ...
    return model_factory(**spec, img_size=img_size, num_classes=num_classes)