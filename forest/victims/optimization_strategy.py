...
class Strategy:
    ...
    def __init__(self, model_name, args):
        ...
        if 'levit' in model_name.lower() or 'swin' in model_name.lower():
            self.epochs = 100
            self.batch_size *= 4 if 'levit' in model_name.lower() else 2
            self.lr *= 0.1
            self.validate = 25
        ...
        self.milestones = [self.epochs // 2.667, self.epochs // 1.6, self.epochs // 1.142]
        if 'swin' in model_name.lower():
            self.milestones = [self.epochs // 2, self.epochs // 1.333, self.epochs // 1.111]
