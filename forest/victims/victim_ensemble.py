...
class _VictimEnsemble(_VictimBase):
    ...
    def initialize(self, seed=None, silent=False):
        if self.args.memory_cache:
            self.ensemble_setup = {'device': torch.device('cpu')}
        else:
            self.ensemble_setup = {'device': self.setup['device']}
        ...
        self.models, self.definitions, self.criterions, self.optimizers, self.schedulers, self.epochs, self.stats = [], [], [], [], [], [], []
        ...
        self.epoch = 0
        self.epochs = [0 for _ in range(self.args.ensemble)]
    ...
    def _iterate(self, kettle, poison_delta, max_epoch=None):
        ...
        multi_model_setup = (self.models, self.definitions, self.criterions, self.optimizers, self.schedulers, self.stats)
        ...
        if poison_delta is None and self.args.stagger:
            ...
            stagger_list = [max_epoch for _ in range(self.args.ensemble)]

        run_stats = list()
        for idx, single_model in enumerate(zip(*multi_model_setup)):
            model, defs, criterion, optimizer, scheduler, stats = single_model
            single_setup = (model, self.feature_extractor, self.defs, criterion, optimizer, scheduler)
            # Move to GPUs
            model.to(**self.setup)
            ...
            for epoch in range(stagger_list[idx]):
                self._step(kettle, poison_delta, loss_fn, epoch, stats, *single_setup)
                if self.args.dryrun:
                    break
            # Return to CPU
            if torch.cuda.device_count() > 1:
                model = model.module
            model.to(**self.ensemble_setup)
    ...
    def step(self, kettle, poison_delta, poison_targets, true_classes):
        ...
        """
        multi_model_setup = (self.models, self.criterions, self.optimizers, self.schedulers, self.stats)

        run_stats = list()
        for idx, single_model in enumerate(zip(*multi_model_setup)):
            model, criterion, optimizer, scheduler, stats = single_model
            ...
            self._step(kettle, poison_delta, loss_fn, self.epochs[idx], stats, *single_setup, silent=True)
            run_stats.append(stats)
            self.epochs[idx] += 1
            model.to(**self.ensemble_setup)
    ...
    def compute(self, function, *args):
        ...
        for idx, (model, criterion, optimizer) in enumerate(zip(self.models, self.criterions, self.optimizers)):
            with GPUContext(self.setup, model) as model:
                ...
                outputs.append(function(model, self.feature_extractor, criterion, optimizer, *single_arg))
        # collate
