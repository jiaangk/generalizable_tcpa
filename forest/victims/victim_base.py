...
class _VictimBase:
    ...
    def __init__(self, args, setup=dict(device=torch.device('cpu'), dtype=torch.float)):
        ...
        if self.feature_extractor:
            self.feature_extractor.to(**self.setup)
    ...
    def validate(self, kettle, poison_delta):
        ...
        run_stats = list()
        if not (kettle.augmentations or self.feature_extractor is None):
            kettle.trainset_feature_reconstruction(poison_delta)
            poison_delta = None
    ...
    def _initialize_model(self, model_name):
        if self.args.transfer:
            if self.args.pretrained is None:
                raise ValueError('When conduct end-to-end transfer learning experiments, we should assign a pretrained model to the adversary.')
            
            pretrained_head = pretrained_picker(self.args.pretrained, self.args)
            if not isinstance(pretrained_head, torch.nn.Module):
                raise ValueError('This model is not supported to conduct end-to-end transfer learning.')
            
            model = get_model(model_name, self.args.dataset, self.args, pretrained=self.args.pretrained)
            model = torch.nn.Sequential(pretrained_head, model)
    ...
    def _get_feature_extractor(self):
        if self.args.pretrained is None or self.args.transfer:
            return None
        return pretrained_picker(self.args.pretrained, self.args)

    def _step(self, kettle, poison_delta, loss_fn, epoch, stats, model, feature_extractor, defs, criterion, optimizer, scheduler, silent=False):
        """Single epoch. Can't say I'm a fan of this interface, but ..."""
        run_step(kettle, poison_delta, loss_fn, epoch, stats, model, feature_extractor, defs, criterion, optimizer, scheduler, silent=silent)
