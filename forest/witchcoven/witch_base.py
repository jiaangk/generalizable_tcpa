...
class _Witch():
    ...
    def __init__(self, args, setup=dict(device=torch.device('cpu'), dtype=torch.float)):
        ...
        self.retain = True if (self.args.ensemble > 1 and self.args.local_rank is None) or self.args.recipe == 'trajectory-matching' else False
    ...
    def brew(self, victim, kettle):
        ...
        if len(kettle.poisonset) > 0:
            if len(kettle.datasets.targetset) > 0:
                if self.args.eps > 0:
    ...
    def _get_gradient(self, victim):
        # Precompute target gradients
        if self.args.recipe in ('distribution-matching', 'gradient-matching'):
    ...
    def _initialize_brew(self, victim, kettle):
        ...
        self.targets = torch.stack([kettle.targetset[idx][0] for idx in range(len(kettle.targetset))]).to(**self.setup)
        self.intended_classes = torch.tensor([kettle.poison_setup['intended_class'] for _ in range(len(kettle.targetset))]).to(device=self.setup['device'], dtype=torch.long)
        self.true_classes = torch.tensor([kettle.targetset[idx][1] for idx in range(len(kettle.targetset))]).to(device=self.setup['device'], dtype=torch.long)

        self._get_gradient(victim)
    ...
    def _run_trial(self, victim, kettle):
        ...
        if self.args.attackoptim in ['Adam', 'signAdam', 'momSGD', 'momPGD']:
            ...
            if self.args.scheduling:
                ...
                # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(att_optimizer, T_max=self.args.attackiter, eta_min=self.tau0 * 0.001)
            poison_delta.grad = torch.zeros_like(poison_delta).to(**self.setup)
            dm, ds = kettle.dm, kettle.ds
            poison_bounds = torch.zeros_like(poison_delta).to(**self.setup)
        else:
            poison_bounds = None
        ...
        for step in range(self.args.attackiter):
            ...
            if self.args.attackoptim in ['Adam', 'signAdam', 'momSGD', 'momPGD']:
                ...
                with torch.no_grad():
                    # Projection Step
                    poison_delta.data = torch.clamp(poison_delta, -self.args.eps / ds / 255, self.args.eps / ds / 255)
                    poison_delta.data = torch.clamp(poison_delta, -dm / ds - poison_bounds, (1 - dm) / ds - poison_bounds)
            ...
            if self.args.retrain:
                if step % (self.args.retrain_iter) == 0 and step != 0 and step != (self.args.attackiter - 1):
                    ...
                    delta = poison_delta.detach()
                    
                    victim.initialize()
                    print('Model reinitialized to random seed.')
                    
                    victim._iterate(kettle, poison_delta=delta)
                    print('Retraining done!')
                    self._get_gradient(victim)

            if self.args.step:
                stats = victim.step(kettle, poison_delta, self.targets, self.true_classes)
                self._get_gradient(victim)

            if self.args.dryrun:
                break

        poison_delta = poison_delta.to(device=torch.device('cpu'))
    ...
    def _batched_step(self, poison_delta, poison_bounds, example, victim, kettle):
        ...
        if len(batch_positions) > 0:
            ...
            if self.args.attackoptim in ['PGD', 'GD']:
                ...
                # Return slice to CPU:
                poison_delta[poison_slices] = delta_slice.detach()
    ...
    def _pgd_step(self, delta_slice, poison_imgs, tau, dm, ds):
        ...
        with torch.no_grad():
            ...
            # Projection Step
            delta_slice.data = torch.clamp(delta_slice, -self.args.eps / ds / 255, self.args.eps / ds / 255)
            delta_slice.data = torch.clamp(delta_slice, -dm / ds - poison_imgs, (1 - dm) / ds - poison_imgs)
